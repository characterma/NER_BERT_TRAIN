...
# from keyword_matching import get_keyword_extractor
...
def tag_char(df, args):
    def _tag_char(example, param_args):
        content = str(example['input_text']) if pd.notna(example['input_text']) else ""
        tag = ['O'] * len(str(content))

        for entity_type in param_args.entity_type:
            pos_list = []
            entity_list = example[f"{entity_type}_gt"]
            if entity_list == []:
                continue
            else:
    ...
    df['tag_char'] = df.progress_apply(_tag_char, param_args=args, axis=1)
    return df
...
def generate_train_and_dev_dataloader(train_dir_path, valid_dir_path, batch_size):
    # load the data
    train_set = datasets.load_from_disk(train_dir_path)
    valid_set = datasets.load_from_disk(valid_dir_path)
    
    # define the dataloader
    # data_collator = default_data_collator
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=batch_size, collate_fn=data_collator, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(valid_set, batch_size=batch_size, collate_fn=data_collator, shuffle=False)
    
    return train_loader, valid_loader


def generate_test_dataloader(test_dir_path, batch_size):
    # load the data
    test_set = datasets.load_from_disk(test_dir_path)

    # define the dataloader
    # data_collator = default_data_collator
    test_loader = torch.utils.data.DataLoader(test_set, batch_size=batch_size, collate_fn=test_data_collator, shuffle=False)
...
def convert_label_to_entity(df, entity_types):
    def _convert_label_to_entity(example, entity_types):
        output = {f"{label}_pred": [] for label in entity_types}
    ...
    df_output = pd.DataFrame(df.progress_apply(_convert_label_to_entity, entity_types=entity_types, axis=1).to_list())

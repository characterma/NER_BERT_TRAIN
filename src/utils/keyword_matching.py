...
# KeywordProcessor code from flashtext==2.7
            ...
            if char not in self.non_word_boundaries:
                ...
                if self._keyword in current_dict or char in current_dict:
                    ...
                    if longest_sequence_found:
                        ...
                        if self._keyword in current_dict_continued \
                            and current_dict_continued[self._keyword] != longest_sequence_found:
                            keywords_extracted.append((current_dict_continued[self._keyword], sequence_start_pos, idy-1))
...
class KeyWordExtractor(object):
    def __init__(self, dictionary=None, case_sensitive=True):
        if isinstance(dictionary, str):
            assert os.path.isfile(dictionary), 'There is not dictionary file.'
            self.dictionary = [data.rstrip() for data in open(dictionary, encoding='utf-8')]
    ...
    def extract(self, text, span_info=True):
        result = self.keyword_processor.extract_keywords(text, span_info=span_info)
        return result


def filter_keywords(keywords, merge_by_index=False):
    """
    过滤关键词列表并根据需求决定是否按照索引位置合并包含关系的关键词。
    
    参数：
    keywords (list of tuple): 关键词列表，每个关键词是一个三元组 (keyword, start_index, end_index)。
    merge_by_index (bool): 是否按照索引位置合并包含关系的关键词。
    
    返回：
    list of tuple: 过滤后的关键词列表。
    """
    if keywords and isinstance(keywords[0], str):
        ...
        if merge_by_index:
            ...
            unique_keywords = list(set(keyword[0] for keyword in keywords))
                
    if merge_by_index:
        unique_keywords.sort(key=lambda x: (x[1], x[2]))
        ...
        for kw in unique_keywords:
            if not merged_keywords:
                ...
                last_kw = merged_keywords[-1]
                if last_kw[1] <= kw[1] and last_kw[2] >= kw[2]:
                    continue
                elif kw[1] <= last_kw[1] and kw[2] >= last_kw[2]:
                    merged_keywords[-1] = kw
                else:
                    merged_keywords.append(kw)
        
        merged_keywords = set(item[0] for item in merged_keywords)
    else:
        merged_keywords = set()
        ...
        for i in unique_keywords:
            ...
            for j in unique_keywords:
                if i != j and i in j:
                    ...
                    break
            if is_unique:
                merged_keywords.add(i)
    
    return list(merged_keywords)


if __name__ == '__main__':
    file_path = "tmp/coffee_tea_keyword_0625.txt"
    text = '转给首页!!快来薅!!//@买买小卖部:一次可以拍五份,拍下13天内有效!过 star 期 starbucks 自动退!外卖直接送到家(免配送费)[泪]。转给首页！！快来薅！！//@买买小卖部:一次可以拍五份，拍下13天内有效！过期自动退！外卖直接送到家（免配送费）[泪]_x000d_卧槽。。。星巴克这么便宜了？？？ 摩卡可可星星巴克摩卡可可星冰乐39块钱两大杯'

    # Extractor = KeyWordExtractor(dictionary=file_path, case_sensitive=False)
    # keywords = Extractor.extract(text, span_info=True)
    # print(keywords)

    kw_list = ["星冰乐", "星冰乐", "星巴克", "摩卡可可", "摩卡可可星冰乐", "Star", "starbucks"]
    Extractor = KeyWordExtractor(dictionary=kw_list, case_sensitive=False)
    keywords = Extractor.extract(text, span_info=True)
    print(keywords)

    keywords2 = filter_keywords(keywords, merge_by_index=True)
    print(keywords2)
    keywords3 = filter_keywords(keywords, merge_by_index=False)
    print(keywords3)